...
def initialize_or_update_vector_store():
    ...
    embeddings = HuggingFaceEmbeddings(
        ...
        model_kwargs={"device": "mps"}  # Use macOS Metal for speed
    )
...
def chat():
    """Handle chat requests and return answers based on documents."""
    if qa_chain is None:  # Initialize QA system if not already done
        setup_qa_system()
    query = request.json.get("query")  # Get query from POST request
    if not query:
        return jsonify({"error": "No query provided"}), 400  # Return error if query is empty
    
    logger.info(f"Processing query: {query}")
    result = qa_chain.invoke({"query": query})  # Run the query through the QA chain
    logger.info(f"Retrieved {len(result['source_documents'])} docs for query: {query}")
    logger.info(f"Response: {result['result']}")
    logger.info(f"Source documents: {[doc.page_content[:200] for doc in result['source_documents']]}")
    
    # Return response and source documents as JSON
    return jsonify({"response": result["result"], "sources": [doc.page_content for doc in result["source_documents"]]})

# Debug route to list loaded documents
@app.route("/debug/documents", methods=["GET"])
def debug_documents():
    """Return a JSON list of loaded documents for debugging."""
...
# Route to manually refresh the vector store
@app.route("/refresh", methods=["POST"])
def refresh_vector_store():
    """Refresh the vector store with new or changed documents."""
    global vector_store, qa_chain
    vector_store = initialize_or_update_vector_store()
    qa_chain = None  # Reset QA chain to reinitialize with updated store
    return jsonify({"message": "Vector store refreshed successfully."})

# Main entry point
if __name__ == "__main__":
    try:
        setup_qa_system()  # Set up the system on startup
